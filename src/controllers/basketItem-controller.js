...
module.exports = {
  addBasketItem: async (req, res, next) => {
    try {
      ...
      const foundBasketItem = await basketItemService.findBasketIdByBookId(
        req.body.basketId,
        req.body.bookId
      );
      if (!foundBasketItem) {
        ...
        const data =
          await basketItemService.updateAmountBasketItemWhenAddSameBook(
            foundBasketItem,
            value.amount
          );
  ...
  getAllBasketItemFromBasketId: async (req, res, next) => {
    try {
      const data = await basketItemService.getAllListInBasket(
        req.user.basketId
      );
  ...
  clearMyCartInBasketItem: async (req, res, next) => {
    try {
        // console.log(req.body)
        const { basketId } = req.params
    //   console.log(basketId);
    await basketItemService.updateAllBasketItemByBasketId(+basketId)
    return res.status(200).json({
        message : "Clear Cart Complete"
    })
    } catch (err) {
      next(err);
    }
